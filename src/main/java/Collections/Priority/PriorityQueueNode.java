...
public class PriorityQueueNode<T> implements Comparable<PriorityQueueNode> {
    ...
    public int compareTo(PriorityQueueNode obj) {
        ...
        // Casting the incoming object to PriorityQueueNode<T>
        PriorityQueueNode<T> temp = obj;

        // Compare based on priority first
        if (priority > temp.getPriority()) {
            result = 1; // Higher priority
        } else if (priority < temp.getPriority()) {
            result = -1; // Lower priority
        } else {
            // If priorities are equal, compare based on order
            if (order > temp.getOrder()) {
                result = 1; // Higher order
            } else {
                result = -1; // Lower order
            }
        }
        return result;
    }
